...
Author: Joshua Gray
Date: 10/11/21
Description: This program sorts arrays with generic methods that use quick sort.
*/
...
public class Exercise23_03 {
  ...
  public static <E extends Comparable<E>> int partition(E[] list, int first, int last) {
	  ...
	  while (high > low) {
		  ...
		  while (low <= high && (list[low].compareTo(pivot) == -1 || 
				  list[low].compareTo(pivot) == 0)) {
			  low++;
		  }
  ...
  public static <E> int partition(E[] list, Comparator<? super E> comparator, int first, int last) {
	  ...
	  while (high > low) {
		  // Search forward from left
		  while (low <= high && (comparator.compare(list[low], pivot) == -1 || 
				  comparator.compare(list[low], pivot) == 0)) {
			  low++;
		  }
	  ...
	  while (high > first && (comparator.compare(list[high], pivot) == 1 || 
			  comparator.compare(list[high], pivot) == 0)) {
		  high--;
	  }
